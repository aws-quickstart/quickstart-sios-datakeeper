...
try {
    ...
    $sqlInstallCmds = [ordered]@{
        "SQL2017" = '/ACTION="AddNode" /SkipRules=Cluster_VerifyForErrors Cluster_IsWMIServiceOperational /ENU="True" /Q /UpdateEnabled="False" /ERRORREPORTING="False" /USEMICROSOFTUPDATE="False" /UpdateSource="MU" /HELP="False" /INDICATEPROGRESS="False" /X86="False" /INSTANCENAME="SIOSSQLSERVER" /SQMREPORTING="False" /FAILOVERCLUSTERGROUP="SQL Server (SIOSSQLSERVER)" /CONFIRMIPDEPENDENCYCHANGE=' + $isMultiSubnet + ' /FAILOVERCLUSTERIPADDRESSES=' + $Addresses + ' /FAILOVERCLUSTERNETWORKNAME="siossqlserver" /AGTSVCACCOUNT=' + $SQLFULLServiceAccount + ' /SQLSVCACCOUNT=' + $SQLFULLServiceAccount + ' /FTSVCACCOUNT="NT Service\MSSQLFDLauncher" /SQLSVCPASSWORD=' + $SQLServiceAccountPassword + ' /AGTSVCPASSWORD=' + $SQLServiceAccountPassword + ' /SQLSVCINSTANTFILEINIT=False /FTSVCACCOUNT="NT Service\MSSQLFDLauncher`$SIOSSQLSERVER" /IAcceptSQLServerLicenseTerms /SUPPRESSPRIVACYSTATEMENTNOTICE=True /IACCEPTPYTHONLICENSETERMS=False /IACCEPTROPENLICENSETERMS=False';
    }

    $exitcode = $NULL
    $InstallSqlPs={
        Param($DomainAdminFullUser, $SQLFULLServiceAccount, $SQLServiceAccountPassword, $isMultiSubnet, $Addresses, $SQLServerVersion, $arguments)
    ...
    while (($Retries -lt 20) -and (!$Installed)) {
        ...
        $Retries++
        Invoke-Command -Authentication Credssp -Scriptblock $InstallSqlPs -ComputerName $NetBIOSName -Credential $DomainAdminCreds -ArgumentList $DomainAdminFullUser,$SQLFULLServiceAccount,$SQLServiceAccountPassword,$isMultiSubnet,$Addresses,$SQLServerVersion,$sqlInstallCmds[$SQLServerVersion]
        $Installed = $true
    }
